...
-- {{{ Key bindings
globalkeys = gears.table.join(awful.key({}, 'XF86AudioRaiseVolume', volume_widget.raise, {
    ...
    scratch.toggle("kitty --class quake", {
        instance = "quake"
    })
    ...
    description = "show the window switcher",
    group = "launcher"
}))

clientkeys = gears.table.join(awful.key({modkey}, "w", function(c)
    c:kill()
end, {
    description = "close",
    group = "client"
